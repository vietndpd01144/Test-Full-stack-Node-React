...
import { signInApi, signOutApi, signUpApi } from '@api/user/useApi';
...
export const signInAction = createAsyncThunk<SignInResponse, LoginDataForm, { rejectValue: SignInError }>(
    '/user/sign-in',
...
export const signUpAction = createAsyncThunk<SignUpResponse, SignUpDataForm, { rejectValue: SignUpError }>(
    '/user/sign-up',
    async (payload, { rejectWithValue }) => {
        try {
...
export const signOutAction = createAsyncThunk('/user/sign-out', async () => {
...
export const userSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(signInAction.rejected, (state, { payload }) => {
            ...
            toast.error(data?.messages ? data?.messages[0] : data?.message ?? 'Sign in fail.');
        ...
        builder.addCase(signUpAction.rejected, (state, { payload }) => {
            const data = payload?.response.data;
            toast.error(data?.messages ? data?.messages[0] : data?.message ?? 'Sign up fail.');
